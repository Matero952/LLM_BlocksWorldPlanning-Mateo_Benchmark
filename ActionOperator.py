...
class betterAction:
    ...
    #TODO May need to write function turning model output into correctly formatted action.
    #Some helper functions
    def __write_action_operator__(self):
        start_state = ast.literal_eval(self.start_state)
        end_state = ast.literal_eval(self.end_state)
        action = ast.literal_eval(self.action)
    ...
    def get_operator_bindings(self) -> dict:
        ...
        return op_bindings
    #Helper function that finds start state, end state based on problem pddl file.
    ...
    # TODO Create helper function that takes in problem file and returns start state and end states.
if __name__ == "__main__":
    example_action = "{""pick"": ""None"", ""place"": ""None""}"
    ba = betterAction('problem.pddl', example_action, 'hehe.pddl.soln')
...
#
#
# class ActionOperator(betterAction):
#     def __init__(self, output_path, action: Action, domain_file, problem_file, pick_action: bool):
#         super().__init__(output_path=output_path, action=action)
#         self.action = action
